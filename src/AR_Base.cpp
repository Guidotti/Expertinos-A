...
void Aprendizagem_Reforco::AssignQValue( unsigned long long* IndexesCMAC, unsigned long long IndexAct, int numAct, double Reforco )
{
	double maxQ = -DBL_MAX;
	...
	for( int i = 0; i < lut_m; i++ )
	{
		index = indiceCMAC_ant[i];
		MatrizQ[index][numAct] = MatrizQ[index][numAct] + AlfaQ*( Reforco + GamaQ*maxQ - MatrizQ[index][numAct] );
	}
}
...
void Aprendizagem_Reforco::saveMatrixQ( int playerNum )
{
	...
	else // "Successful in opening the file, saving values of array MatrizQ"
	{
		...
		cerr << "Matrix info: " << "(" << difZero << "/" << numElementos << ") = "
				<< (difZero/((double)numElementos*1.0))*100.0 << "% explored for player " << playerNum << "\n";
	}
	Fout.close();
}
